...
select.prototype = {
  ...
  create  : function() {
    ...
    this.markBackground = this.game.add.image(75,221,'choose_bg_mark');
    this.markBackground.anchor.setTo(0,0);

    this.markSelectedX = this.game.add.image(83, 252, 'mark_selected');
    this.markSelectedX.anchor.setTo(0, 0);
    this.markSelectedX.alpha = 0;

    this.markSelectedO = this.game.add.image(187, 252, 'mark_selected');
    this.markSelectedO.anchor.setTo(0, 0);
    this.markSelectedO.alpha = 0;


    this.xMark = this.game.add.button(83, 252, 'cell', this.selectMarkX, this,1,1,1);
    this.xMark.anchor.setTo(0, 0);

    this.oMark = this.game.add.button(187, 252, 'cell', this.selectMarkO, this,2,2,2);
    this.oMark.anchor.setTo(0, 0);


    this.startButton = this.game.add.button(75, 374, 'startbutton',this.startGame, this, 1, 1, 2);
    this.startButton.anchor.setTo(0, 0);
    this.startButton.inputEnabled = false ;

    this.startButtonDisabled = this.game.add.image(75, 374, 'startbutton_disabled', this);
